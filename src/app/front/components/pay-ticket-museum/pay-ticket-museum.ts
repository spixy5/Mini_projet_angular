...
calculateTotalAmount() {
  ...
  if (!visitDetails || !this.museum) return ;
  const ticketType=visitDetails.ticketType || 'adult';
  const ticketCount=visitDetails.ticketCount || 1;
  ...
  this.totalTicketsAmount=(price-discountAmount)*ticketCount
    this.totalAmount=this.totalTicketsAmount-this.reduction(price*ticketCount)+2;
}
